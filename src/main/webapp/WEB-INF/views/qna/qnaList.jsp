			...
			<tbody>
				<c:forEach items="${qnaList}" var="qna">
					<tr class="text-center">
						<c:forEach begin="1" end="${qna.boGroupDepth}" var="i">
						<td></td>
						</c:forEach>
						<td class="text-left">
							<c:if test="${qna.boDelYn eq 'Y' }"> 원작자에 의해 삭제된 댓글입니다 </c:if>
							<c:if test="${qna.boDelYn eq 'N' }"> <a href="qnaView.wow?boNo=${qna.boNo}" >${qna.boTitle}</a></c:if>
							--<a href="qnaForm.wow?boGroupNo=${qna.boGroupNo}&boGroupDepth=${qna.boGroupDepth}">
								답글쓰기</a></td>
					</tr>
				</c:forEach>
			</tbody>
